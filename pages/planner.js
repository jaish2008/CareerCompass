...
const STORAGE_KEY = "careerCompassPlanner";
const STREAK_KEY = "careerCompassStreak";
const XP_KEY = "careerCompassXP";
  ...
  return `
    <div class="task-card ${task.done ? "done" : ""}" data-id="${task.id}">
      ...
      <div class="task-meta">
        ...
        <span class="priority-pill priority-${task.priority.toLowerCase()}"><span class="priority-dot"></span>${task.priority}</span>
      </div>
      ${!task.done ? `<button class="focus-btn">▶ Start Focus Session</button>` : ""}
    </div>
  `;
}
...
function renderCategoryBreakdown() {
  ...
  box.innerHTML = Object.entries(counts).map(([cat, c]) => `
    <div class="category-row"><span>${cat}</span><strong>${c.done}/${c.total}</strong></div>
  `).join("");
}
...
function renderNotifications() {
  ...
  list.innerHTML = notifs.length
    ? notifs.map(n => `<div class="notif-item ${n.type}">${n.text}</div>`).join("")
